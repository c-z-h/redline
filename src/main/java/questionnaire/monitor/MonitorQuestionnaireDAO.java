...
public class MonitorQuestionnaireDAO extends JdbcDaoSupport implements QuestionnaireDAO {

	@Override
	public int insertQuestinnaire(QuestionnaireIntf questionnaire) {
		String sql = "insert into monitor_questionnaire(longitude, latitude, weight) values(?,?,?)";
	...
	public double getWeightByLL(double longitude, double latitude) {
		String sql="select weight from monitor_questionnaire where longitude=? and latitude=?";
